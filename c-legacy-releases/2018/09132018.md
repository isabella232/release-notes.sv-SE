...
## [!DNL Experience Cloud] och [!UICONTROL Core Services]{#coreservices}
...
* Uppdaterade [!UICONTROL Mobile] servicerelaterade URL:er från [!DNL *.marketing.adobe.com] till [!DNL *.experiencecloud.adobe.com].
...
Nu kan du ändra det färgschema som används i Workspace genom att välja från en annan färgpalett eller ange en egen palett. Den här funktionen påverkar många saker i Workspace, bland annat de flesta visualiseringar. It **does not** affect Summary Change, conditional formatting in Freeform tables, and the Map visualization.

Obs! Stöd för färgpaletter har inte aktiverats för Internet Explorer 11.

**Ny mall för arbetsyta: Ljudförbrukning**

Läs mer i [Ljudanalys](https://marketing.adobe.com/resources/help/en_US/sc/appmeasurement/hbvideo/media-workspace-templates.html).

**Användarbehörigheter**

* Borttagen _Skapa/kuratera projekt med[!UICONTROL Analysis Workspace]_ behörighet, vilket hindrade vissa användare från att skapa ett nytt Workspace-projekt (d.v.s. visa den blå knappen Skapa) eller kuratera komponenter för projekt.
...
**Förbättring av Advertising Analytics**

Introducerade det [automatiska spårningsläget](https://marketing.adobe.com/resources/help/en_US/analytics/advertising/aa_manual_vs_automatic_tracking.html) för automatisk spårning av sökmotordata i er Adobe Analytics-implementering.

Förbättrad användning av serversamtal

En ny larmtyp för _[serversamtal](https://marketing.adobe.com/resources/help/en_US/analytics/servercall/scu_alerts.html)_ har lagts till, som erbjuder proaktiva meddelanden när användningen av serversamtal närmar sig åtagandenivåerna.
...
* **Bearbetningsregler**: Ett problem som uppstod när en eVar inaktiverades på rapportsvitsnivå har åtgärdats. Den inaktiverade eVar visades inte längre i listrutan och alla referenser till den eVar i bearbetningsreglerna standarderas sedan till sidans URL. Om eVar som du använder i en regel är inaktiverad tar vi inte bort den från listrutan och ändrar inte regeln. (AN-164565)
...
* **[!UICONTROL Reports & Analytics]**: Korrigerade ett problem med schemalagda projekt som inte kunde levereras efter datamigrering. (AN-165800)
* **[!UICONTROL Reports & Analytics]**: Korrigerade ett problem med visade bilder i schemalagda kontrollpaneler i Outlook. (AN-165302)
* **[!UICONTROL Reports & Analytics]**: Ett problem som gjorde att bokmärkeshanteraren inte kunde visas i det japanska användargränssnittet har korrigerats. (AN-156948, AN-147271)
* **[!UICONTROL Reports & Analytics]**: Korrigerade ett problem som förhindrade användare från att använda Skapa segment från utfallssökväg i utfallsrapporten. (AN-166976)
* **[!UICONTROL Report Builder]**: Korrigerade ett problem som uppstod när kornighet skulle användas varje vecka i en [!UICONTROL Report Builder] begäran med beräknade värden - samma data returnerades för alla radobjekt för varje mätvärde. Samma problem kan också ha inträffat i Web Services API och i [!UICONTROL Reports & Analytics] Reporting. (AN-166754).
...
I Versionshistorik [för](https://docs.adobe.com/content/help/sv-SE/analytics/implementation/appmeasurement-updates.html) AppMeasurement finns en versionshistorik för AppMeasurement på följande plattformar:
...
Eftersom stödet för TLS 1.0 har upphört rekommenderar vi att användare av Adobe [!UICONTROL Report Builder] (ARB) laddar ned ARB v5.6.21 före den 13 september 2018. **Efter detta datum kommer tidigare versioner av ARB inte längre att fungera**.
...
>[!IMPORTANT]
>
> Uppdatera din installation av [Report Builder](https://marketing.adobe.com/resources/help/en_US/arb/t_install_arb.html) till den senaste versionen. Den här uppdateringen är en förutsättning för att du ska kunna migrera Analytics-användar-ID:t till Admin Console, med början i april 2018.
...
| Funktion | Beskrivning |
|--- |--- |
| Meddelanden i appen (beta) | Med meddelanden i appen kan ni engagera [!UICONTROL Mobile] appanvändare mer effektivt genom att tillhandahålla sammanhangsberoende interaktion och göra det möjligt att nå användare som har valt att inte använda push-meddelanden. Använd meddelanden i appen tillsammans med push-meddelanden för att skapa en personaliserad och relevant upplevelse. Detta leder till bättre konvertering och lojalitet hos appanvändarna. |
| Adobe Launch-integrering för mobilappar (beta) | Lanseringsintegrering med Adobe Campaign förenklar och automatiserar nu processen för aktivering av [!UICONTROL Mobile] appegenskaper i Campaign med hjälp av [!UICONTROL Mobile] SDK V5. |
...
Läs mer om Adobe Campaign här: [Adobe Digital Library](https://digitalu.adobe.com/content/Enablement/en.html).
...
* [Användarhandbok för AEM Assets Brand Portal](https://helpx.adobe.com/se/experience-manager/brand-portal/user-guide.html)
...
Dokumentationen för utveckling av [SPA-program (Single page applications)](https://helpx.adobe.com/experience-manager/6-4/sites/developing/user-guide.html?topic=/experience-manager/6-4/sites/developing/morehelp/spa.ug.js) för AEM SPA Editor uppdateras och utökas.
...
Adobe har avvecklat tjänsten på [https://marketing.adobe.com/](https://marketing.adobe.com/).
...
**[!UICONTROL Mobile]Bildgenerator för signeringsverktyg**

Ett nytt [bildgenereringsverktyg för mobilsignering](https://helpx.adobe.com/digital-publishing-suite/help/mobile-signing-tool-image-generator.html) är nu tillgängligt att använda. Med det här verktyget kan du generera de bilder som behövs, med exakt de dimensioner och namn som krävs AEM [!UICONTROL Mobile] signeringsverktyget.

**[!UICONTROL Mobile]-Android-push-meddelanden**

Push- [meddelandena i dokumentationen för AEM Mobile-appar](https://helpx.adobe.com/digital-publishing-solution/help/push-notifications.html#push-android) har nu uppdaterats för att inkludera stegen för att använda Google Firebase (Android)-konsolen.
...
Evenemanget i år arrangeras i Schinkelhalle och kommer att vara en interaktiv communityupplevelse. Våra sponsorer ger er en plattform för att möta några av de mest kända utvecklarna Sling, Jackrabbit och Felix. Ytterligare sessioner handlar om företagsimplementationer som Adobe Experience Manager, med fokus på hur arkitekturen utnyttjas i dessa produkter.

Glöm inte Hackaton! Ta kontakt med de bästa och smartaste mobilnätsinställningarna i communitymedlemskapet i Apache/Adobe. Högtalarna får rabatt utan extra kostnad.
