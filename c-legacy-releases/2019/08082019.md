...
| -----------| ---------- |  
| Stöd för inställningar för Samma webbplats-cookie | Inställningen [](https://web.dev/samesite-cookies-explained) Samma webbplats-cookie läggs till i alla cookies som anges av Analytics. Med den här ändringen kan du vara kompatibel med Chrome-ändringar som kräver cookie-fältet SameSite. Analytics cookies används som standard `none`. Om du har använt en domän som tillhör första part (t.ex. stats.domain.com) exklusivt kan du låta Adobe Customer Care ange den som `lax` för samlingsdomäner från första part. |
...
| A4T Aktivitetsimpression och aktivitetskonverteringar aktiverade för attribuerings-IQ | Vi har aktiverat två A4T-mått (Analytics for Target) för attribuerings-IQ: Aktivitetspåståenden och aktivitetskonvertering. Tidigare var mätvärdena i Analysis Workspace uppblåsta jämfört med Reports &amp; Analytics. Med den här förändringen kan användare nu använda en&quot;samma touch&quot;-attribueringsmodell, som gör att Analysis Workspace följer rapportens och Analytics riktlinjer. |
...
[De nya funktionerna](https://helpx.adobe.com/campaign/kb/control-panel-instance-settings.html) i Kontrollpanelen innefattar möjligheten att lägga till URL:er som Campaign Classic ansluter till för data-/filöverföringar.

Observera att det [!UICONTROL Control Panel] finns både Adobe Campaign Classic- och Adobe Campaign Standard-kunder på AWS. Du behöver inte uppgradera för att komma åt Kontrollpanelen.
...
* Annonsörer med Google Ads-konton som är berättigade till kundmatchning kan nu göra följande:
   ...
   * Uppdatera alla Google-kundmatchningar utom målgrupper som skapats av en Adobe-publik. Du kan överföra data som ska läggas till, tas bort eller ersätta alla befintliga data för målgruppen. All kontaktinformation måste hashas med hjälp av SHA-256-algoritmen.
