...
| **Felsökning** | När du är inloggad med din Adobe ID kan du nu välja att hämta efterbearbetade träffar i Experience Cloud Debugger. Efterbearbetade träffar är serveranrop efter att de har gått igenom [!UICONTROL Processing Rules] och VISTA-regler, vilket gör att du kan validera [!UICONTROL Processing Rules] och dina VISTA-regler. **Obs**: Om du använder A4T (SupplementalDataID) kan det ta några minuter att komma tillbaka efter bearbetningen av data. |
...
| **Ny användarhandbok för Tech Notes** | Användarhandboken [Tech Notes](https://docs.adobe.com/content/help/en/analytics/technotes/home.html) är nu tillgänglig. För närvarande syftar programmet till att hjälpa användare med mer erfarenhet av analysverktyg från tredje part, som Google Analytics, att bli mer bekanta med Adobe Analytics. Användarhandboken för tekniska anteckningar kommer att byggas ut under de närmaste månaderna för att innehålla ytterligare innehåll. |
...
* Ett problem med funktionen **[!UICONTROL Set as Landing Page]** som inte fungerar när den delas med en stor uppsättning mottagare har korrigerats. (AN-163922)
...
* Ett problem som uppstod när en avisering med måttet **[!UICONTROL Exit Rate]** skapades har korrigerats. (AN-177476)

### Viktiga meddelanden för Analytics-administratörer{#aa-notices} 

| Meddelande | Datum tillagt eller uppdaterat | Beskrivning |
| -----------| ---------- | ---------- |
| Uppdatering av datainsamling | Lagt till 27 juni 2019 | Alla Adobe Analytics-cookies med flaggan&quot;same-site-by-default-cookies&quot; har nu flaggan `SameSite=None`. |
| Ändringar i användargränssnittet för granskningslogg | Uppdaterad 25 juni 2019 | I API `Logs.GetUsageLog` gjordes följande ändringar i gränssnittet: <ul><li>Svarshuvuden i API ändrades: a) från `timestamp` till `dateCreated`, b) från `login` till `companyLogin`, c) från `event_num` till `eventType`, d) från `event_type` till `event_type`, e) från `ip_address` till `ipAddress`, f) från `report_suite` till `rsid`, g) från &lt;a a12/> till `logText`.`event_details` </li><li>Tidsstämpelformatet har ändrats från Unix-tidsstämpel till ISO 8601-tid</li></ul><br>I API:t och i användargränssnittet för administratörsloggen innehåller granskningsloggarna nu rapportsvitens ID i stället för rapportsvitens namn. |
...
| Säkra SMS-meddelanden (TLS) | Skyddat SMS stöds nu via den utökade allmänna SMPP-anslutningen. Detta tillåter en krypterad anslutning till providern. <br> Mer information finns i den [detaljerade dokumentationen](https://helpx.adobe.com/campaign/kb/sms-connector-protocol-and-settings.html). |
