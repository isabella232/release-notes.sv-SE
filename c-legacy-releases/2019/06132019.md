...
description: Versionsinformation om juni 2019 för Experience Cloud
...
* Information om de senaste uppdateringarna av [Adobe Experience Platform finns](https://www.adobe.io/apis/experienceplatform/home/services/release-notes.html#!end-user/markdown/release-notes/release-notes-20190515.md) i versionsinformationen [!DNL Experience Platform]för Adobe.io.

### [!DNL Experience Platform Launch]

* Den senaste informationen finns [!DNL Experience Platform Launch](https://docs.adobelaunch.com/) här.
...
* [Viktiga meddelanden för Analytics-administratörer](#aa-notices) **(uppdaterad 27 juni 2019)**
...
| -----------| ---------- |  
| **Segmentering** | Nya attribueringsmodeller för segmenteringsdimensioner:<ul><li>Upprepande (standard): Inkluderar instanser + beständiga värden för dimensionen.</li><li>Instans: Inkluderar instanser för dimensionen.</li><li>Icke upprepande instans: Inkluderar unika instanser (icke-upprepande) för dimensionen.</li></ul> [Mer](https://docs.adobe.com/content/help/en/analytics/components/segmentation/segmentation-workflow/seg-build.html) |
| **Segmentering** | Nya segmentoperatorer: **[!UICONTROL Equals Any of]** och **[!UICONTROL Does not Equal Any of]**. [Mer...](https://docs.adobe.com/content/help/en/analytics/components/segmentation/segment-reference/seg-operators.html) |
| **Felsökning** | När du är inloggad med din Adobe ID kan du nu välja att hämta efterbearbetade träffar i Experience Cloud Debugger. Efterbearbetade träffar är serversamtal efter att de gått igenom [!UICONTROL Processing Rules] och VISTA-regler, vilket gör att du kan validera [!UICONTROL Processing Rules] och dina VISTA-regler. **Obs**: Om du använder A4T (SupplementalDataID) kan det ta några minuter att komma tillbaka efter bearbetningen av data. |
...
| **Analysis Workspace** | Vi har lagt till en varning i utfallsvisualiseringen som visas när du lägger till ett segment som kontaktyta - vissa ogiltiga kombinationer av segmentbehållare leder till ogiltiga utfallsdiagram, som <ul><li>Använda ett besöksbaserat segment som kontaktyta inuti en besökarsammanhangsbaserad utfallsvisualisering</li><li>Använda ett besöksbaserat segment som kontaktyta i en snabbutfallsvisualisering</li><li>Använda ett besöksbaserat segment som kontaktyta inuti en snabbmeny för utfall</li></ul> <br> [Mer...](https://docs.adobe.com/content/help/en/analytics/analyze/analysis-workspace/visualizations/fallout/compare-segments-fallout.html) </br> |
| **Förbättringar av analysdokumentation** | Analysdokumentationen har omstrukturerats och innehåller nu samarbetsfunktioner som gör att du kan förbättra innehållet! Du kan logga problem med dokumentationen och föreslå ändringar. Dokumentuppsättningen har flyttats till en [ny domän](https://docs.adobe.com/content/help/sv-SE/analytics/landing/home.html). Omdirigeringar ska vara på plats. |
| **Ny användarhandbok för Tech Notes** | Användarhandboken [för](https://docs.adobe.com/content/help/en/analytics/technotes/home.html) Tech Notes är nu tillgänglig. För närvarande syftar programmet till att hjälpa användare med mer erfarenhet av analysverktyg från tredje part, som Google Analytics, att bli mer bekanta med Adobe Analytics. Användarhandboken för tekniska anteckningar kommer att byggas ut under de närmaste månaderna för att innehålla ytterligare innehåll. |
...
| -----------| ---------- | ---------- |
| Uppdatering av datainsamling | Lagt till 27 juni 2019 | Alla Adobe Analytics-cookies med flaggan&quot;same-site-by-default-cookies&quot; har nu flaggan inställd på `SameSite=None`. |
| Ändringar i användargränssnittet för granskningslogg | Uppdaterad 25 juni 2019 | I API:t `Logs.GetUsageLog`har följande ändringar gjorts i gränssnittet: <ul><li>Svarshuvuden i API ändrades: a) från `timestamp` till `dateCreated`, b) från `login` till `companyLogin`, c) från `event_num` till `eventType`, d) från `event_type` till `event_type`, e) från `ip_address` till `ipAddress``report_suite` `rsid``event_details` `logText`, f) från ‡ till, g) från Under. </li><li>Tidsstämpelformatet har ändrats från Unix-tidsstämpel till ISO 8601-tid</li></ul><br>I API:t och i användargränssnittet för administratörsloggen innehåller granskningsloggarna nu rapportsvitens ID i stället för rapportsvitens namn. |
...
| Kommande ändring av _rapportsummeberäkningar_ | Uppdaterad 2 maj 2019 | Den 13 **juni 2019** kommer Adobe Analytics att göra beräkningarna av _Report Total_ konsekvent för alla dimensioner och mätvärden. Detta resulterar i en förändring av summorna för vissa rapporter (vanligen rapporter om utkast eller kundattribut). Före den här ändringen inkluderades eller exkluderades _ospecificerade_ radposter i summan för vissa rapportsummor, oavsett om _Ospecificerad_ visades i rapporten. <br/>Från och med den 13 juni 2019 visas _Ospecificerad_ alltid i rapportsumman, även om den inte visas som en radpost i rapporten. Dessutom _finns_ segment som används eller _finns_ inte logik, vilket kan visa olika resultat för vissa dimensioner efter den här ändringen. Den här ändringen påverkar Analysis Workspace, Rapporter och analyser, Ad Hoc Analysis, Report Builder och API:t för rapportering. |
...
* Ett problem som medförde att tillstånd krävdes för att [!DNL VIEW_DATASOURCES] ta bort en partner har åtgärdats.
...
| Guardrail, robuskhet och skalbarhet | En rad förbättringar har lagts till [!DNL Campaign Classic]. Förbättringar av säkerhet, stabilitet och skalbarhet finns i [Adobe Campaign Classic Release Notes](https://docs.adobe.com/content/help/sv-SE/campaign-classic/using/release-notes/latest-release.translate.html). |
